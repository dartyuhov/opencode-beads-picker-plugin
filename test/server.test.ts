...
test("creates native Beads file attachments with structured detail payloads", async () => {
  ...
  assert.deepEqual(attachments.map((part) => part.mime), ["application/pdf", "application/pdf"])
  assert.ok(attachments.every((part) => part.url.startsWith("data:application/pdf;base64,")))
